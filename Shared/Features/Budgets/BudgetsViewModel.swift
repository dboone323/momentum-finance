...
final class BudgetsViewModel {
    ...
    func budgetsForMonth(_ budgets: [Budget], month: Date) -> [Budget] {
        ...
        return budgets.filter { budget in
            calendar.isDate(budget.startDate, equalTo: month, toGranularity: .month)
        }
    }
    ...
    func createBudget(category: ExpenseCategory, limitAmount: Decimal, month: Date) {
        ...
        let budget = Budget(
            ...
            startDate: month,
            endDate: calendar.date(byAdding: DateComponents(month: 1, day: -1), to: month) ?? month,
            category: category
        )
    ...
    func spendingTrend(for category: ExpenseCategory, months: Int = 6) -> [MonthlySpending] {
        ...
        for i in 0..<months {
            ...
            let spent = (category.totalSpent(for: monthDate) as NSDecimalNumber).doubleValue
    ...
    func updateBudgetRolloverSettings(_ budget: Budget, enabled: Bool, maxPercentage: Double) {
        ...
        do {
            try self.modelContext?.save()
            NotificationManager.shared.schedulebudgetWarningNotifications(for: [budget])
        } catch {
    ...
    func applyRolloverToNextPeriod(for budget: Budget) {
        ...
        let calendar = Calendar.current
        guard let nextMonth = calendar.date(byAdding: .month, value: 1, to: budget.startDate) else {
            return
        }
        ...
        let nextPeriodBudgetExists = existingBudgets.contains { existingBudget in
            existingBudget.category?.name == budget.category?.name
                && calendar.isDate(existingBudget.startDate, equalTo: nextMonth, toGranularity: .month)
        }
