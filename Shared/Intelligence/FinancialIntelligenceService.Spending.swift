        ...
        Decimal]]
{
    ...
    for txn in transactions where txn.amount < 0 {
        ...
        monthlySpendingByCategory[categoryName, default: [:]][month] =
            (monthlySpendingByCategory[categoryName]?[month] ?? Decimal(0)) + abs(txn.amount)
    }
    ...
    -> [FinancialInsight]
{
    ...
    for budget in budgets {
        ...
        if let exceededInsight = fi_checkBudgetExceeded(
            budget: budget, totalSpent: totalSpent, categoryId: category.name
        ) {
