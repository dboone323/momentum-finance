...
public actor ExportEngineService: ModelActor {
    ...
    private func generateBudgetsCSV(settings _: ExportSettings) async throws -> String {
        ...
        for budget in budgets {
            ...
            let category = budget.category?.name ?? ""
    ...
    private func generateSubscriptionsCSV(settings _: ExportSettings) async throws -> String {
        ...
        for subscription in subscriptions {
            ...
            let category = subscription.category?.name ?? ""
            let account = ""
    ...
    private func drawTransactionsSummary(
        ...
        settings: ExportSettings
            ...
            "Total Income: $\(String(format: "%.2f", (totalIncome as NSDecimalNumber).doubleValue))"
                .draw(
                    at: CGPoint(x: 70, y: currentY),
                    withAttributes: textAttributes
                )
            ...
            "Total Expenses: $\(String(format: "%.2f", (totalExpenses as NSDecimalNumber).doubleValue))"
                .draw(
                    at: CGPoint(x: 70, y: currentY),
                    withAttributes: textAttributes
                )
            ...
            "Net Amount: $\(String(format: "%.2f", (netAmount as NSDecimalNumber).doubleValue))"
                .draw(
                    at: CGPoint(x: 70, y: currentY),
                    withAttributes: textAttributes
                )
    ...
    private func drawAccountsSummary(
        ...
        settings _: ExportSettings
            ...
            for account in accounts {
                let accountInfo = "\(account.name): $\(String(format: "%.2f", (account.balance as NSDecimalNumber).doubleValue))"
