...
import SwiftUI
    ...
    struct BalanceTrendChart: View {
        ...
        func generateSampleData() -> [(date: String, balance: Double)] {
            [
                ...
                (date: "Jun", balance: (self.account.balance as NSDecimalNumber).doubleValue),
            ]
        }
                ...
                HStack {
                    ...
                    VStack(alignment: .center) {
                        Text("Change")
                            ...
                            .foregroundStyle(.secondary)
                        Text(
                            "+\(((self.account.balance - 1250) as NSDecimalNumber).doubleValue.formatted(.currency(code: self.account.currencyCode)))"
                        )
                        .font(.subheadline)
    ...
    struct CreditAccountDetailsView: View {
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 12) {
                ...
                let creditLimit = ((self.account.creditLimit ?? 0) as NSDecimalNumber).doubleValue
                let balance = abs((self.account.balance as NSDecimalNumber).doubleValue)
                ...
                if let creditLimit = account.creditLimit, creditLimit > 0 {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        ProgressView(value: abs((self.account.balance as NSDecimalNumber).doubleValue), total: (creditLimit as NSDecimalNumber).doubleValue)
                            .tint(
                                self.getCreditUtilizationColor(
                                    used: abs((self.account.balance as NSDecimalNumber).doubleValue), limit: (creditLimit as NSDecimalNumber).doubleValue
                                )
                            )
    ...
    struct AccountEditModel {
        ...
        init(from account: FinancialAccount) {
            ...
            self.balance = (account.balance as NSDecimalNumber).doubleValue
