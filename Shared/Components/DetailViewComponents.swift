...
import SwiftUI
    ...
    struct CategoryTag: View {
        ...
        var body: some View {
            HStack(spacing: 4) {
                Circle()
                    .fill(Color(hex: self.category.colorHex))
            ...
            .padding(.vertical, 2)
            .background(Color(hex: self.category.colorHex).opacity(0.1))
            .cornerRadius(4)
        }
    }
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(alignment: .top) {
                    ...
                    VStack(alignment: .trailing, spacing: 4) {
                        ...
                        let usedPercentage = budgetTotal > 0 ? budgetSpent / budgetTotal : 0
                        Text("\(Int(usedPercentage * 100))% Used")
                            .font(.caption)
                            .foregroundStyle(
                                self.getBudgetColor(usedPercentage)
                            )
                    }
                }

                ProgressView(value: self.budgetSpent, total: self.budgetTotal)
                    .tint(self.getBudgetColor(budgetTotal > 0 ? budgetSpent / budgetTotal : 0))

                HStack {
                    Text(
                        "This transaction: \(Decimal(self.transactionAmount).formatted(.currency(code: "USD")))"
                    )
                    .font(.caption)
                    .foregroundStyle(.secondary)
