...
import SwiftUI
    ...
    struct PaymentHistoryChart: View {
        ...
        func generateSampleData() -> [(month: String, amount: Double)] {
            [
                (month: "Jun", amount: (self.subscription.amount as NSDecimalNumber).doubleValue),
                (month: "Jul", amount: (self.subscription.amount as NSDecimalNumber).doubleValue),
                (month: "Aug", amount: (self.subscription.amount as NSDecimalNumber).doubleValue),
                (month: "Sep", amount: (self.subscription.amount as NSDecimalNumber).doubleValue),
                (month: "Oct", amount: (self.subscription.amount as NSDecimalNumber).doubleValue),
                (month: "Nov", amount: (self.subscription.amount as NSDecimalNumber).doubleValue),
            ]
        }
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 8) {
                Chart {
                    ...
                    RuleMark(y: .value("Average", (self.subscription.amount as NSDecimalNumber).doubleValue))
    ...
    struct ValueAssessmentView: View {
        ...
        private var costPerUse: Double {
            ...
            (self.subscription.amount as NSDecimalNumber).doubleValue / 5.0
        }
                ...
                HStack {
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            let lowerBound = ((self.subscription.amount * 0.9) as NSDecimalNumber).doubleValue
                                .formatted(.currency(code: self.subscription.currencyCode))
                            let upperBound = ((self.subscription.amount * 1.1) as NSDecimalNumber).doubleValue
                                .formatted(.currency(code: self.subscription.currencyCode))
                            Text(
    ...
    struct SubscriptionEditModel {
        ...
        init(from subscription: Subscription) {
            ...
            self.amount = (subscription.amount as NSDecimalNumber).doubleValue
