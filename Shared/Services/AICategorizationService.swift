...
struct AICategorizationService {
    ...
    static func predictCategory(for title: String, categories: [ExpenseCategory]) -> ExpenseCategory? {
        ...
        for category in categories {
            ...
            if catName.contains("food") || catName.contains("grocer") || catName.contains("dining") { matchedKey = "food" }
            else if catName.contains("transport") || catName.contains("car") || catName.contains("auto") { matchedKey = "transportation" }
            else if catName.contains("util") || catName.contains("bill") { matchedKey = "utilities" }
            else if catName.contains("entertain") || catName.contains("fun") { matchedKey = "entertainment" }
            else if catName.contains("shop") { matchedKey = "shopping" }
            else if catName.contains("health") || catName.contains("med") || catName.contains("well") { matchedKey = "health" }
            else if catName.contains("home") || catName.contains("hous") { matchedKey = "housing" }
            else if catName.contains("travel") || catName.contains("trip") { matchedKey = "travel" }
