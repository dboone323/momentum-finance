...
public final class BudgetAgent: BaseAgent {
    ...
    public func execute(context: [String: any Sendable]) async throws -> AgentResult {
        ...
        let reasoningResult = await performAutonomousReasoning(
            transactions: transactions,
            burnRate: (burnRate as NSDecimalNumber).doubleValue,
            anomalies: anomalies
        )
    ...
    private func performAutonomousReasoning(
        ...
        let transactionSummary = transactions.prefix(5).map { "\($0.note): \($0.amount)" }.joined(separator: ", ")
        let anomalySummary = anomalies.map { "\($0.note): \($0.amount)" }.joined(separator: ", ")
        ...
        do {
            ...
            let cleanedResponse = if let range = response.range(of: "\\{.*\\}", options: options) {
                String(response[range])
            } else {
                response
            }

            guard let data = cleanedResponse.data(using: String.Encoding.utf8),
