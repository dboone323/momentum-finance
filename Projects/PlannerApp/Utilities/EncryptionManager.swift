...
// Generated for PlannerApp on October 28, 2025
...
class EncryptionManager {
    ...
    private let queue = DispatchQueue(label: "com.PlannerApp.encryption", qos: .userInitiated)
    ...
    func getMasterKey() -> SymmetricKey? {
        keychain.retrieveKey(identifier: "master_key")
    }
...
class KeychainManager {
    ...
    private let service = "PlannerApp.encryption"
...
extension EncryptionManager {
    ...
    func encryptUserData(_ userData: Data) -> Data? {
        encryptData(userData, classification: .confidential).success
    }
...
extension Data {
    func encrypted(classification: DataClassification = .confidential) -> Data? {
        EncryptionManager.shared.encryptData(self, classification: classification).success
    }

    func decrypted(classification: DataClassification = .confidential) -> Data? {
        EncryptionManager.shared.decryptData(self, classification: classification).success
    }
}
...
// MARK: - CloudKit Encryption Integration

extension EncryptionManager {
    // Encrypt data before CloudKit sync
    func encryptForCloudKit(_ data: Data, recordType: String) -> Data? {
        let classification: DataClassification
        switch recordType {
        case "UserData", "PersonalInfo":
            classification = .restricted
        case "TaskData", "ProjectData":
            classification = .confidential
        default:
            classification = .internal
        }
        return encryptData(data, classification: classification).success
    }

    // Decrypt data after CloudKit sync
    func decryptFromCloudKit(_ encryptedData: Data, recordType: String) -> Data? {
        let classification: DataClassification
        switch recordType {
        case "UserData", "PersonalInfo":
            classification = .restricted
        ...
        return decryptData(encryptedData, classification: classification).success
    }
}
